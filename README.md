...
- Demo the Streamlit “SBA Copilot” to show quick policy answers and confidence scores.
- Hit the `/underwrite` route with a sample borrower (pre-canned CSV + feature payload) to show how decisions + letters + citations appear instantly.
- Highlight audit logs (`logs/audit`) to prove every action is tracked.
- Position phase 2 as “replace manual spreadsheet underwriting with guided decisions the minute a file arrives.”

Let me know if you’d like a deployment guide, scripted demo notebook, or connection adapters for your existing LOS/CRM.
